...
(async function checkBillAdminClientTest() {
  ...
  async function runTest(testCase) {
    ...
    try {
      ...
      const successSonner = await driver.wait(
        until.elementIsVisible(driver.findElement(sonner), 5000)
      );

      assert.equal(
        await successSonner.getText(),
      ...
      const bills = await driver.findElements(
        By.css("tbody.ant-table-tbody tr.ant-table-row")
      );

      assert.notEqual(bills.length, 0, "Không có đơn hàng xủ lý");
