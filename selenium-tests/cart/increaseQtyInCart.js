...
(async function increaseQtyInCart() {
  ...
  async function runTest(testCase) {
    ...
    try {
      ...
      let cartItemQtyElement;
      let initialQty;

      for (const item of cartItems) {
        cartItemQtyElement = item.findElement(
          By.css("div.w-16.mx-2.text-center")
        );

        item
          .findElement(By.css("div.flex.items-center > button:nth-child(3)"))
          .click();
      }
      ...
      await driver.wait(
        until.elementTextIs(await driver.findElement(sonner), "Increase"),
        1000
      );

      assert.notEqual(await cartItemQtyElement.getText(), initialQty);
